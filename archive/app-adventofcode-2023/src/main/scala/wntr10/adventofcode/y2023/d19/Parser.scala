...
object Parser {
  ...
  class Alpha(var a: Array[Bravo], var b: Charlie)

  class Bravo(var a: Yankee, var b: Delta)

  class Foxtrot(var a: Yankee, var b: Xray)

  type Charlie = Array[Array[Foxtrot]]
  type Delta = Array[Yankee]
  ...
  private def bravo(str: String): String = {
    record(str, '{', List(yankee, delta))
  }

  private def india(str: String): String = {
    array(str, '\n', bravo)
  }
  ...
  private def golf(str: String): String = {
    array(str, ',', yankee)
  }
